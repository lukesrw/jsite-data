...
const ISTYPE_INPUTS: any[] = [[], [false], [true]];
...
let tests: {
    ...
    _getType: {
        cases: {
            ...
            Objects: {
                ...
                "Function instance (class) is an object": [
                    new (MyClass2 as FunctionConstructor)(),
                    ["object", "object", "myclass2"]
                ],
    ...
    _isIn: {
        cases: {
            ...
            NaN: {
                "NaN exists in the container": [NaN, [true, true, true, false]]
            },
            Numbers: {
    ...
    _isInstance: {
        cases: {
            Objects: {
                ...
                "MyClass2 is instance": [new (MyClass2 as FunctionConstructor)(), [false, true]]
            }
        },
...
Object.keys(TYPES).forEach((name, name_i) => {
    let description = `${name} is a${
        VOWELS.indexOf(name.toLowerCase().substr(0, 1)) > -1 ? "n" : ""
    } ${name.toLowerCase()}`;

    let expect = new Array(ISTYPE_INPUTS.length * ISTYPE_INPUT_TYPES * name_i)
        .fill(false)
    ...
    Object.assign(tests._isType.cases, {
        [name]: {
            [`${description}`]: [TYPES[name as keyof typeof TYPES], expect]
        }
    });

    ISTYPE_INPUTS.forEach(inputs => {
        tests._isType.inputs = tests._isType.inputs || [];
        tests._isType.inputs.push(
            [name.toLowerCase()].concat(inputs),
            [[name.toLowerCase()]].concat(inputs),
            [{ [name.toLowerCase()]: true }].concat(inputs)
        );
    });

    ISEQUAL_INPUTS.forEach(inputs => {
        tests._isEqual.inputs = tests._isEqual.inputs || [];
        tests._isEqual.inputs.push([TYPES[name as keyof typeof TYPES]].concat(inputs));
    });
});
        ...
        Object.keys(tests[method].cases).forEach(name => {
            context(name, () => {
                ...
                Object.keys(tests[method].cases[name]).forEach(desc => {
                     ...
                     */
                    if (
                        Array.isArray(tests[method].inputs) &&
                        tests[method].cases[name][desc][1].length < (tests[method].inputs || []).length
                    ) {
                        tests[method].cases[name][desc][1] = tests[method].cases[name][desc][1].concat(
                            new Array(
                                (tests[method].inputs || []).length - tests[method].cases[name][desc][1].length
                            ).fill(tests[method].cases[name][desc][1][tests[method].cases[name][desc][1].length - 1])
                        );
                    }

                    if (tests[method].inputs) {
                        (tests[method].inputs || []).forEach((test, test_i) => {
                            ...
                            it(desc + (test.length > 0 ? `, arguments: ${args}` : ""), () => {
                                expect(
                                    (Data[method as keyof typeof Data] as Function)(
                                        tests[method].cases[name][desc][0],
                                        ...test
                                    )
                                ).to.equal(tests[method].cases[name][desc][1][test_i]);
                            });
                        });
                    }
                });
